<?php
/**
 * The main .INC for the download-non-text-files script.
 *
...
$download_non_text_files_lock_file
	= TrackitStockManagement_FeedFilesHelper
		::get_download_non_text_files_lock_file();

if (
	$download_non_text_files_lock_file->is_locked()
) {
	...
	$files_to_download_count = count($fs);
	...
	$download_non_text_files_lock_file->unlock();
	
	echo date('c') . " $files_to_download_count $successful_downloads_count\n";
}
?>