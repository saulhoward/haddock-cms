...
if (
	...
	||
	isset($_GET['last_deleted_shopping_basket_id'])
	||
	...
	||
	isset($_GET['stock_not_available_exception'])
) {
	if (
		isset($_GET['last_deleted_product_id']) 
		&&
		 ...
		 * DEPRECATED
		 *
		 * See the if clause for 'last_deleted_shopping_basket_id'.
		 */
	...
	} elseif (isset($_GET['last_deleted_shopping_basket_id'])) {
		#echo "Deleted shopping basket.\n"; exit;
		
		$message_div
			= Shop_ShoppingBasketsTableRenderer
				::get_deleted_shopping_basket_confirmation_div_for_sbid(
					$_GET['last_deleted_shopping_basket_id']
				);
	...
	} elseif (isset($_GET['stock_not_available_exception'])) {
		$message_div = new HTMLTags_Div();
		
		#if (isset($_SESSION['stock-not-available-exception'])) {
		#	$message_div->append_str_to_content($_SESSION['stock-not-available-exception']->getMessage());
		#} else {
			$message_div_p = new HTMLTags_P('Sorry, that stock is not available.');
			$message_div->append_tag_to_content($message_div_p);
		#}
	}
	
	$message_div->set_attribute_str('id', 'last_action_div');
...
## SPACER DIV
$spacer_div = new HTMLTags_Div();
$spacer_div->set_attribute_str('style', 'height:400px;');
$content_div->append_tag_to_content($spacer_div);
