<?php
/**
 * The content of the suppliers page.
 *
...
$table_renderer = $suppliers_table->get_renderer();
...
if (isset($_GET['delete_all'])) {
	...
	$question_delete_all_p = new HTMLTags_P('Are you sure that you want to delete all of the suppliers?');
	 ...
	 */
	$row = $suppliers_table->get_row_by_id($_GET['delete_id']);

	$question_p = new HTMLTags_P();

	$question_p->set_attribute_str('class', 'question');

	$question_p->append_str_to_content('Are you sure that you want to delete this supplier?');
	 ...
	 * Row editing.
	 */
	$row = $suppliers_table->get_row_by_id($_GET['edit_id']);
	...
	$row_adding_form = $table_renderer->get_supplier_adding_form($row_adding_url, $cancel_href);
				...
				|| isset($_GET['deleted_all'])) 
	{
		...
		if (isset($_GET['last_deleted_id'])) {
			$message = 'Deleted supplier id: ' . $_GET['last_deleted_id'];
		}
		elseif (isset($_GET['last_edited_id'])) {
			$message = 'Edited supplier id: ' . $_GET['last_edited_id'];
		}
		elseif (isset($_GET['last_added_id'])) {
			$message = 'Added supplier id: ' . $_GET['last_added_id'];
		}
		elseif (isset($_GET['deleted_all'])) {
			...
			{
				$message = 'Failed to delete all of your suppliers.';
			}
		}
		$last_error_box_div
	...
	$add_row_a = new HTMLTags_A('Add New Supplier');
	...
	$delete_all_a = new HTMLTags_A('Delete All Suppliers');
	...
	$limit_form->add_hidden_input('page', 'suppliers');
	...
	$row_count = $suppliers_table->count_all_rows();
	...
	#$fields = $suppliers_table->get_fields();
	#
	#foreach ($fields as $field) {
	#    $heading_row->append_sortable_field_name($field->get_name());
	#}

	$field_names = explode(' ', 'name contact_name notes');
	...
	$telephone_header = new HTMLTags_TH('Telephone Number'); 
	...
	$rows = $suppliers_table->get_all_rows(ORDER_BY, DIRECTION, OFFSET, LIMIT);

	foreach ($rows as $row) {
		$row_renderer = $row->get_renderer();

		#$data_tr = $row_renderer->get_admin_database_tr();
		$data_tr = $row_renderer->get_admin_suppliers_html_table_tr($current_page_url);
