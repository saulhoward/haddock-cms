<?php
/**
 * The content of the shopping_baskets page.
 *
...
$table_renderer = $shopping_baskets_table->get_renderer();
...
if (isset($_GET['delete_all'])) {
	...
	$question_delete_all_p = new HTMLTags_P('Are you sure that you want to delete all of the shopping baskets?');
	 ...
	 */
	$row = $shopping_baskets_table->get_row_by_id($_GET['delete_id']);
	 ...
	 * Row editing.
	 */
	$row = $shopping_baskets_table->get_row_by_id($_GET['edit_id']);
				...
				|| isset($_GET['deleted_all'])) 
	{
		...
		elseif (isset($_GET['last_edited_id'])) {
			$message = 'Edited product id: ' . $_GET['last_edited_id'];
		}
		...
		elseif (isset($_GET['deleted_all'])) {
			...
			if ($_GET['deleted_all'] == 'successful')
			{
				$message = 'Succesfully deleted 
					all of your shopping baskets! 
					(Not really - feature disabled)';
			}
			else
			{
				$message = 'Failed to delete all of your shopping baskets.';
			}
		}
		$last_error_box_div
	...
	$delete_all_a = new HTMLTags_A('Delete All shopping baskets');
	...
	$limit_form->add_hidden_input('page', 'shopping-baskets');
	...
	$row_count = $shopping_baskets_table->count_all_rows();
	...
	#$fields = $shopping_baskets_table->get_fields();
	#
	#foreach ($fields as $field) {
	#    $heading_row->append_sortable_field_name($field->get_name());
	#}

	$field_names = explode(' ', 'added session_id quantity deleted');
	...
	$rows = $shopping_baskets_table->get_all_rows(ORDER_BY, DIRECTION, OFFSET, LIMIT);

	foreach ($rows as $row) {
		$row_renderer = $row->get_renderer();

		#$data_tr = $row_renderer->get_admin_database_tr();
		$data_tr = $row_renderer->get_admin_shopping_basket_html_table_tr($current_page_url);
