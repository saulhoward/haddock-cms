<?php
/**
 * The content of the photo graphs admin page for
 * the shop plug-in.
 *
...
$photographs_table_renderer = $photographs_table->get_renderer();
...
if (isset($_GET['delete_id'])) {
	 ...
	 */
	$row = $photographs_table->get_row_by_id($_GET['delete_id']);
	...
	$question_p->append_str_to_content(
		'Are you sure that you want to delete this photograph?'
	);
	...
	$content_div
        ->append_tag_to_content(
            $row_renderer->get_admin_photograph_html_table()
        );
	 ...
	 * Row editing.
	 */
	$photograph_row = $photographs_table->get_row_by_id($_GET['edit_id']);
	$photograph_row_renderer = $photograph_row->get_renderer();
	$row_editing_form
        = $photograph_row_renderer
            ->get_photograph_editing_form(
                $redirect_script_url,
                $cancel_href
            );
	...
	$row_adding_url->set_get_variable('add_photograph');

	$row_adding_form
        = $photographs_table_renderer
            ->get_photograph_adding_form(
                $row_adding_url,
                $cancel_href
            );
	...
	if (
		...
		if (isset($_GET['last_deleted_id'])) {
			$message = 'Deleted photograph id: ' . $_GET['last_deleted_id'];
		}
		elseif (isset($_GET['last_edited_id'])) {
			$message = 'Edited photograph id: ' . $_GET['last_edited_id'];
		}
		elseif (isset($_GET['last_added_id'])) {
			$message = 'Added photograph id: ' . $_GET['last_added_id'];
		}
		$last_error_box_div
	...
	$add_row_a = new HTMLTags_A('Add New Photograph');
	...
	#$delete_all_href->set_get_variable('page', 'photographs');
...
//        $limit_form->add_hidden_input('page', 'photographs');
	...
	$row_count = $photographs_table->count_all_rows();
	...
	$field_names = explode(' ', 'name added');
	...
	foreach (
		$photographs_table_renderer->get_admin_database_action_ths()
		as
		$action_th
	) {
