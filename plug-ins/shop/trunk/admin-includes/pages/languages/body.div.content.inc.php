<?php
/**
 * The content of the languages page.
 *
 * From here, the user can
 *
 *  - add a new language
 *  - delete a language
 *  - Rearrange the sort order of languages
 *  - Edit a language
 * 
...
$table_renderer = $languages_table->get_renderer();
...
if (isset($_GET['delete_all'])) {
	...
	$question_delete_all_p = new HTMLTags_P('Are you sure that you want to delete all of the languages?');
	 ...
	 */
	$row = $languages_table->get_row_by_id($_GET['delete_id']);
	 ...
	 * Row editing.
	 */
	$row = $languages_table->get_row_by_id($_GET['edit_id']);
	...
	#$row_adding_action->set_get_variable('page', 'languages');
	#$row_adding_action->set_get_variable('table', $languages_table->get_name());
	...
	$row_adding_form = $table_renderer->get_language_adding_form($row_adding_url, $cancel_href);
...
The ISO 639-1 language code is a two letter unique identifier for language. Eg.
TXT;
				...
				|| isset($_GET['deleted_all'])) 
	{
		...
		if (isset($_GET['last_deleted_id'])) {
			$message = 'Deleted language id: ' . $_GET['last_deleted_id'];
		}
		elseif (isset($_GET['last_edited_id'])) {
			$message = 'Edited language id: ' . $_GET['last_edited_id'];
		}
		elseif (isset($_GET['last_added_id'])) {
			$message = 'Added language id: ' . $_GET['last_added_id'];
		}
		elseif (isset($_GET['deleted_all'])) {
			...
			{
				$message = 'Failed to delete all of your languages.';
			}
		}
		$last_error_box_div
	...
	$add_row_a = new HTMLTags_A('Add New language');
	...
	$delete_all_a = new HTMLTags_A('Delete All languages');
...
//        $limit_form->add_hidden_input('page', 'languages');
	...
	$row_count = $languages_table->count_all_rows();
	...
	#$fields = $languages_table->get_fields();
	#
	...
	$rows = $languages_table->get_all_rows(ORDER_BY, DIRECTION, OFFSET, LIMIT);

	foreach ($rows as $row) {
		$row_renderer = $row->get_renderer();

		#$data_tr = $row_renderer->get_admin_database_tr();
		$data_tr = $row_renderer->get_admin_languages_html_table_tr($current_page_url);
