<?php
/**
 * The content of the customers page.
 *
...
$table_renderer = $customers_table->get_renderer();
...
if (isset($_GET['delete_all'])) {
	...
	$question_delete_all_p = new HTMLTags_P('Are you sure that you want to delete all of the customers?');
	 ...
	 */
	$row = $customers_table->get_row_by_id($_GET['delete_id']);
	 ...
	 * Row editing.
	 */
	$row = $customers_table->get_row_by_id($_GET['edit_id']);
	...
	$row_adding_form = $table_renderer->get_customer_adding_form($row_adding_url, $cancel_href);
				...
				|| isset($_GET['deleted_all'])) 
	{
		...
		if (isset($_GET['last_deleted_id'])) {
			$message = 'Deleted customer id: ' . $_GET['last_deleted_id'];
		}
		elseif (isset($_GET['last_edited_id'])) {
			$message = 'Edited customer id: ' . $_GET['last_edited_id'];
		}
		elseif (isset($_GET['last_added_id'])) {
			$message = 'Added customer id: ' . $_GET['last_added_id'];
		}
...
//        $limit_form->add_hidden_input('page', 'customers');
	...
	$row_count = $customers_table->count_all_rows();
	...
	#$fields = $customers_table->get_fields();
	#
	#foreach ($fields as $field) {
	#    $heading_row->append_sortable_field_name($field->get_name());
	#}

	$field_names = explode(' ', 'added');

	foreach ($field_names as $field_name) {
		$heading_row->append_sortable_field_name($field_name);
	}

	$full_name_header = new HTMLTags_TH('Full Name'); 
	$heading_row->append_tag_to_content($full_name_header);

	$currency_header = new HTMLTags_TH('Address'); 
	$heading_row->append_tag_to_content($currency_header);

	$address_header = new HTMLTags_TH('Email Address'); 
	$heading_row->append_tag_to_content($address_header);

	$telephone_number_header = new HTMLTags_TH('Telephone Number'); 
	$heading_row->append_tag_to_content($telephone_number_header);

	$customer_region_header = new HTMLTags_TH('Customer Region'); 
	...
	$rows = $customers_table->get_all_rows(ORDER_BY, DIRECTION, OFFSET, LIMIT);

	foreach ($rows as $row) {
		$row_renderer = $row->get_renderer();

		#$data_tr = $row_renderer->get_admin_database_tr();
		$data_tr = $row_renderer->get_admin_customers_html_table_tr($current_page_url);
