<?php
/**
 * The content of the currencies page.
 *
 * From here, the user can
 *
 *  - add a new currency
 *  - delete a currency
 *  - Rearrange the sort order of currencies
 *  - Edit a currency
 * 
...
$table_renderer = $currencies_table->get_renderer();
...
if (isset($_GET['delete_all'])) {
	...
	$question_delete_all_p = new HTMLTags_P('Are you sure that you want to delete all of the currencies?');
	 ...
	 */
	$row = $currencies_table->get_row_by_id($_GET['delete_id']);
	 ...
	 * Row editing.
	 */
	$row = $currencies_table->get_row_by_id($_GET['edit_id']);
...
TXT;
				...
				|| isset($_GET['deleted_all'])) 
	{
		...
		if (isset($_GET['last_deleted_id'])) {
			$message = 'Deleted currency id: ' . $_GET['last_deleted_id'];
		}
		elseif (isset($_GET['last_edited_id'])) {
			$message = 'Edited currency id: ' . $_GET['last_edited_id'];
		}
		elseif (isset($_GET['last_added_id'])) {
			$message = 'Added currency id: ' . $_GET['last_added_id'];
		}
		elseif (isset($_GET['deleted_all'])) {
			...
			if ($_GET['deleted_all'] == 'successful')
			{
				$message = 'Succesfully deleted 
					all of your currencies! 
					(Not really - feature disabled)';
			}
			else
			{
				$message = 'Failed to delete all of your currencies.';
			}
		}
		$last_error_box_div
	...
	$add_row_a = new HTMLTags_A('Add New Currency');
	...
	$delete_all_a = new HTMLTags_A('Delete All Currencies');
...
//        $limit_form->add_hidden_input('page', 'currencies');
	...
	$row_count = $currencies_table->count_all_rows();
	...
	#$fields = $currencies_table->get_fields();
	#
	#foreach ($fields as $field) {
	#    $heading_row->append_sortable_field_name($field->get_name());
	#}

	$field_names = explode(' ', 'name iso_4217_code symbol');
	...
	$rows = $currencies_table->get_all_rows(ORDER_BY, DIRECTION, OFFSET, LIMIT);

	foreach ($rows as $row) {
		$row_renderer = $row->get_renderer();

		#$data_tr = $row_renderer->get_admin_database_tr();
		$data_tr = $row_renderer->get_admin_currencies_html_table_tr($current_page_url);
