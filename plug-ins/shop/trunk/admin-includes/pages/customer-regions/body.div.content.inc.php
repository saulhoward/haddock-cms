<?php
/**
 * The content of the customer_regions page.
 *
 * From here, the user can
 *
 *  - add a new customer_region
 *  - delete a customer_region
 *  - Edit a customer_region
 * 
...
$table_renderer = $customer_regions_table->get_renderer();
...
if (isset($_GET['delete_all'])) {
	...
	$question_delete_all_p = new HTMLTags_P('Are you sure that you want to delete all of the customer_regions?');
	 ...
	 */
	$row = $customer_regions_table->get_row_by_id($_GET['delete_id']);

	$question_p = new HTMLTags_P();

	$question_p->set_attribute_str('class', 'question');

	$question_p->append_str_to_content('Are you sure that you want to delete this customer region?');
	 ...
	 * Row editing.
	 */
	$row = $customer_regions_table->get_row_by_id($_GET['edit_id']);
	...
	$row_adding_form = $table_renderer->get_customer_region_adding_form($row_adding_action, $cancel_href);

	$content_div->append_tag_to_content($row_adding_form);
}  elseif (
	isset($_GET['suppliers'])
		&&
		 ...
		 * Set Prices
		 */
		$suppliers_editing_url = clone $redirect_script_url;
		$suppliers_editing_url->set_get_variable('suppliers');
		$suppliers_editing_url->set_get_variable('customer_region_id', $_GET['customer_region_id']);

		$customer_region_supplier_links_table = $database->get_table('hpi_shop_customer_region_supplier_links');
		$customer_region_supplier_links_table_renderer 
			= $customer_region_supplier_links_table->get_renderer();
		$customer_region_supplier_links_setting_form = 
			$customer_region_supplier_links_table_renderer->
			get_customer_region_supplier_links_editing_form(
				$_GET['customer_region_id'],
				$suppliers_editing_url,
				$cancel_href
			);

		$content_div->append_tag_to_content($customer_region_supplier_links_setting_form);
	} else {
		...
		|| isset($_GET['last_edited_id']) 
			|| isset($_GET['last_added_id']) 
				|| isset($_GET['deleted_all'])
					||isset($_GET['last_set_suppliers_for_customer_region_id'])) 
	{

		if (isset($_GET['last_deleted_id'])) {
			$message = 'Deleted Customer Region with id ' . $_GET['last_deleted_id'];
		}
		elseif (isset($_GET['last_edited_id'])) {
			$customer_region_row = $customer_regions_table->get_row_by_id($_GET['last_edited_id']);
			$customer_region_name = $customer_region_row->get_name();
			$message = 'Edited ' . $customer_region_name ;
		}
		elseif (isset($_GET['last_added_id'])) {
			$customer_region_row = $customer_regions_table->get_row_by_id($_GET['last_added_id']);
			$customer_region_name = $customer_region_row->get_name();
			$message = 'Added ' . $customer_region_name;
		}
		elseif (isset($_GET['last_set_suppliers_for_customer_region_id'])) {
			$customer_region_row = $customer_regions_table
				->get_row_by_id($_GET['last_set_suppliers_for_customer_region_id']);
			$customer_region_name = $customer_region_row->get_name();
			$message = 'Changed the suppliers for ' 
		...
		elseif (isset($_GET['deleted_all'])) {
			...
			if ($_GET['deleted_all'] == 'successful')
			{
				$message = 'Succesfully deleted 
					all of your customer regions! 
					(Not really - feature disabled)';
			}
			else
			{
				$message = 'Failed to delete all of your customer regions.';
			}
		}
		$last_error_box_div
	...
	$add_row_a = new HTMLTags_A('Add New Customer Region');
	...
	$delete_all_a = new HTMLTags_A('Delete All Customer Regions');
...
//        $limit_form->add_hidden_input('page', 'customer-regions');
	...
	$row_count = $customer_regions_table->count_all_rows();
	...
	#$fields = $customer_regions_table->get_fields();
	#
	#foreach ($fields as $field) {
	#    $heading_row->append_sortable_field_name($field->get_name());
	#}

	$field_names = explode(' ', 'name description');
	...
	$address_header = new HTMLTags_TH('Language'); 
	$heading_row->append_tag_to_content($address_header);

	$supplier_header = new HTMLTags_TH('Suppliers'); 
	$heading_row->append_tag_to_content($supplier_header);

	$heading_row->append_sortable_field_name('sort_order');

	$set_supplier_header = new HTMLTags_TH('Set Suppliers'); 
	$heading_row->append_tag_to_content($set_supplier_header);
	...
	$rows = $customer_regions_table->get_all_rows(ORDER_BY, DIRECTION, OFFSET, LIMIT);

	foreach ($rows as $row) {
		$row_renderer = $row->get_renderer();

		#$data_tr = $row_renderer->get_admin_database_tr();
		$data_tr = $row_renderer->get_admin_customer_regions_html_table_tr($current_page_url);
