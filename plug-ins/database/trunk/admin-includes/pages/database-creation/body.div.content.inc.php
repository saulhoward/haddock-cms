<?php
/**
 * The Content of the page for Creating Databases.
 *
...
if (!$project_specific_directory->has_passwords_file()) {
    ...
    if (!$root_mysql_user->has_password()) {
        ...
        if (!$root_mysql_user->has_database()) {
            ...
            $create_db_url->set_file('/admin/redirect-script.php');
            $create_db_url->set_get_variable('module', 'database');
            $create_db_url->set_get_variable('page', 'database-creation');
            $create_db_url->set_get_variable('create_db');
            ...
            $db_already_p = new HTMLTags_P();
            
            $db_already_p->append_str_to_content(
                'The database for this project exists.'
            );
            
            $content_div->append_tag_to_content($db_already_p);
        }
    }
}
