<?php
/**
 * FeedAggegator_FeedAggegator
 *
 * @copyright 2008-10-14, SANH
 *
 * FeedAggegator Feeds structure which grabs the xml file and 
	...
	FeedAggegator_FeedAggegator
{
	...
	public function
		__construct(
			$url, // FeedAggegator absolute url
			$version // 'Atom' or 'FeedAggegator2' so we know which class to use
			)
	{
		$timeout = 1;
		...
		else
		{
			...
			{
				$class_to_return = 'FeedAggegator_AtomSimpleXMLElement';
			}
			elseif (strtolower($version) == 'rss2')
			{
				$class_to_return = 'FeedAggegator_FeedAggegatorSimpleXMLElement';
