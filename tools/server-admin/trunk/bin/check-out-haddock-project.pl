...
check-out-haddock-project.pl

=head1 SYNOPSIS

    $ ./check-out-haddock-project.pl -p PROJECT -d DIR

=head1 DESCRIPTION

Checks out the appropriate branch of haddock and the
trunk of the project.

It also checks out any branches of plug-ins that are relevant.
...
Default: L<https://svn.clearlinewebdesign.com/repos>
...
# Check out the branch of haddock for this project.
unless (-d "$directory/haddock") {
    print_and_do_cmd("mkdir \"$directory/haddock\"");
    print_and_do_cmd("svn co $url/haddock/branches/$project \"$directory/haddock\"");
}

# Check out the project specific code.
unless (-d "$directory/project-specific") {
    print_and_do_cmd("mkdir \"$directory/project-specific\"");
    print_and_do_cmd("svn co $url/projects/$project/trunk \"$directory/project-specific\"");
}
